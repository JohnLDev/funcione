...
**Architecture:** Preserve `VITE_AUTH_REDIRECT_URL` as the optional explicit redirect target. When it is empty or omitted, the Supabase auth gateway uses the current browser origin so localhost ports used by Vite/Codex work without changing code. Production keeps an explicit deploy URL, and AdSense remains enabled with the existing public IDs.
...
Set `.env.example` to leave `VITE_AUTH_REDIRECT_URL` blank with comments explaining the current-origin fallback. Set `.env.production` to keep `VITE_AUTH_REDIRECT_URL=https://funcione.pages.dev` and `VITE_ADS_ENABLED=true`.

- [x] **Step 5: Update docs**

Update auth and AdSense docs so localhost testing uses blank `VITE_AUTH_REDIRECT_URL` or an explicit local origin, deploy uses the production URL, and AdSense production remains enabled per the current decision.
